...
class SimpleWidgetConfigureViewModelTest {
    ...
    private val networkInteractor = mock<RuuviNetworkInteractor>()
    private val widgetPreferencesInteractor = mock<WidgetPreferencesInteractor>()
    private val preferencesRepository = mock<PreferencesRepository>()

    @Before
    fun setUp() {
        whenever(networkInteractor.signedIn).thenReturn(false)
        whenever(preferencesRepository.getBackgroundScanInterval()).thenReturn(5)
        whenever(preferencesRepository.getBackgroundScanMode())
            .thenReturn(BackgroundScanModes.DISABLED)
    }
    ...
    fun `sensor without supported measurements cannot be saved`() {
        ...
        verify(widgetPreferencesInteractor, never())
            .saveSimpleWidgetSettings(any(), any(), any())
    }
    ...
    fun `restored unsupported measurement is replaced and the fallback is saved`() {
        ...
        val regularSensor = sensor("regular", UnitType.TemperatureUnit.Celsius)
        val viewModel = createViewModel(regularSensor)
        whenever(widgetPreferencesInteractor.getSimpleWidgetSensor(appWidgetId))
            .thenReturn(regularSensor.id)
        whenever(widgetPreferencesInteractor.getSimpleWidgetType(appWidgetId))
            .thenReturn(WidgetType.AIR_QUALITY)

        viewModel.setWidgetId(appWidgetId)
        viewModel.save()

        assertEquals(WidgetType.TEMPERATURE, viewModel.widgetType.value)
        verify(widgetPreferencesInteractor).saveSimpleWidgetSettings(
            appWidgetId,
            regularSensor.id,
    ...
    fun `stale restored sensor cannot be saved`() {
        ...
        whenever(widgetPreferencesInteractor.getSimpleWidgetSensor(appWidgetId))
            .thenReturn("missing")
        whenever(widgetPreferencesInteractor.getSimpleWidgetType(appWidgetId))
            .thenReturn(WidgetType.TEMPERATURE)
    ...
    private fun createViewModel(vararg sensors: RuuviTag): SimpleWidgetConfigureViewModel {
        whenever(tagRepository.getFavoriteSensors()).thenReturn(sensors.toList())
