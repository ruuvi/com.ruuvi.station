...
class LocalDatabase {
    ...
    class Migration42SensorsShareList(table: Class<SensorsShareList>) : AlterTableMigration<SensorsShareList>(table) {
        override fun onPreMigrate() {
            super.onPreMigrate()
            addColumn(SQLiteType.INTEGER, "isPending")
        }
    }

    @Migration(version = 41, database = LocalDatabase::class)
    class Migration41SensorSettings(table: Class<SensorSettings>) : AlterTableMigration<SensorSettings>(table) {
        override fun onPreMigrate() {
            super.onPreMigrate()
            addColumn(SQLiteType.INTEGER, "backgroundTimestamp")
        }
    }
    ...
    class Migration38Alerts(table: Class<Alarm>) : AlterTableMigration<Alarm>(table) {
        override fun onPreMigrate() {
            super.onPreMigrate()
    ...
    @Migration(version = 39, database = LocalDatabase::class)
    class Migration38SensorSettings(table: Class<SensorSettings>) : AlterTableMigration<SensorSettings>(table) {
        override fun onPreMigrate() {
            super.onPreMigrate()
            addColumn(SQLiteType.INTEGER, "lastUpdated")
            addColumn(SQLiteType.INTEGER, "defaultDisplayOrderTimestamp")
            addColumn(SQLiteType.INTEGER, "displayOrderTimestamp")
        }
    }
